 ...
 * 
 * 1. "[prefix]-name" - Normally an attributed named "data-name", but the prefix is configurable.
 * 2. "[prefix]-collection" - Normally an attributed named "data-name", but the prefix is configurable.
...
export class Selector {
    ...
    one(idOrselector: string): HTMLElement {
        if (idOrselector.substring(0, 1) === "#") {
            const el2 = this.scope.querySelector(idOrselector);
            if (!el2) {
        ...
        if (idOrselector.substring(0, 1) === "$") {
            ...
            const result = this.scope.querySelector(`[${this.prefix}-name='${idOrselector}'],[${this.prefix}-collection='${idOrselector}'],[name="${idOrselector}"],#${idOrselector}`);
            if (result)
                return <HTMLElement>result;
        }

        const item = <HTMLElement>this.scope.querySelector(idOrselector);
        if (!item)
    ...
    all(selector: string): HTMLElement[] {
        ...
        if (selector.substring(0, 1) === "$") {
            ...
            const queryStr=`[${this.prefix}-name='${selector}'],[${this.prefix}-collection='${selector}'],[name="${selector}"],#${selector}`;
            const items2 = this.scope.querySelectorAll(queryStr);
